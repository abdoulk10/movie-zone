...
function RandomMoviePage() {
  ...
  useEffect(() => {
    if (selectedGenre) {
      ...
      fetch(
        `${API_URL}/discover/movie?api_key=ade9ac2663bdc8bc0eae7b07d7787d12&with_genres=${selectedGenre}`
      )
        .then((response) => {
          if (!response.ok) {
  ...
  const handleGenreSelection = (genreId) => {
    ...
    fetch(
      `${API_URL}/discover/movie?api_key=ade9ac2663bdc8bc0eae7b07d7787d12&with_genres=${genreId}`
    )
      .then((response) => {
        if (!response.ok) {
          throw new Error("Network response was not ok");
